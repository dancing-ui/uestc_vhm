...
void RocksDB::open(std::string const &save_path) {
...
void RocksDB::erase(std::string const &key) {
    rocksdb::Status status = this->db_ptr->Delete(rocksdb::WriteOptions(), key);
    ThrowErrorIf(!status.ok(), "Error in RocksDB::delete ...");
}

void RocksDB::get(std::string const &key, bool &value) {
    std::string str_val;
    this->get(key, str_val);
    std::istringstream(str_val) >> value;
}

void RocksDB::get(std::string const &key, int32_t &value) {
    std::string str_val;
    this->get(key, str_val);
    value = std::stol(str_val);
}

void RocksDB::get(std::string const &key, int64_t &value) {
    std::string str_val;
    this->get(key, str_val);
    value = std::stoll(str_val);
}

void RocksDB::get(std::string const &key, std::vector<float> &value) {
    std::string str_val;
    this->get(key, str_val);
    utils::Base64Decode(str_val, value);
}

void RocksDB::get(std::string const &key, std::string &value) {
...
void RocksDB::put(std::string const &key, std::vector<float> const &value) {
    std::string str_val;
    utils::Base64Encode(value, str_val);
    this->put(key, str_val);
}

void RocksDB::put(std::string const &key, bool value) {
    std::string str_val = std::to_string(value);
    this->put(key, str_val);
}

void RocksDB::put(std::string const &key, int32_t value) {
    std::string str_val = std::to_string(value);
    this->put(key, str_val);
}

void RocksDB::put(std::string const &key, int64_t value) {
    std::string str_val = std::to_string(value);
    this->put(key, str_val);
}

void RocksDB::put(std::string const &key, std::string const &value) {
