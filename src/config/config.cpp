...
int32_t Config::Parse() {
    ...
    if (json_obj.contains("model")) {
        if (json_obj["model"].contains("yolo")) {
            ...
            if (item.contains("src_width")) {
                model_cfg_item_.param.src_w = item["src_width"].get<int32_t>();
            } else {
                PRINT_ERROR("'src_width' key not found\n");
                return -9;
            }

            if (item.contains("src_height")) {
                model_cfg_item_.param.src_h = item["src_height"].get<int32_t>();
            } else {
                PRINT_ERROR("'src_height' key not found\n");
                return -10;
            }
